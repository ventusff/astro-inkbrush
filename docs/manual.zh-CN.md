...
`[[` 自动补全(只列本篇所属语种的页面,按该语种里链接的写法给出 id、标题、
品牌名、别名;输入其他语种前缀如 `[[de/` 才列那个语种)、⌘/Ctrl+Enter 保存、
Esc 取消。每次保存过两道关:
...
「块」就是笔记 Markdown 的顶层单元——段落、标题、列表、表格、代码围栏、独立
公式、callout、组件。两种块在渲染出来的位置编辑,而不是在书写的位置:
**脚注定义**自成一块,悬停页脚脚注列表里的那一条即可编辑(预览显示定义正文,
前面带 `[^标签]`);`.md` 笔记里的**裸 HTML 块**从它渲染出的元素上编辑(MDX
里它是组件块)。组件块按源码编辑,没有预览。内容仓 CI 会核对每篇笔记的块
地图(`check-content`:行号戳格式正确、互不重叠、覆盖每一个块)。
...
|---|---|---|
| 凭密码 | 有链接还得有密码 | 不收录 |
| 有链接就能看 | 拿到链接的人——猜不出的 id 就是钥匙 | 不收录 |
| 完全公开 | 所有人;可以有一个可读地址 `<publicBase>/<地址>/`,默认取笔记 id | 收录、canonical、进站点地图 |

1. **创建**——凭密码时弹层预生成 10 位密码(可改;最少 6 位);完全公开时可填
   地址(小写字母、数字、中间的连字符,最多 64 个字符;留空就用 id 地址)。
   有效期 7 天 / 30 天 / 永久三选一(凭密码默认 7 天,另两种默认永久)。
   服务端用站点自己装的 astro、白名单环境变量、10 分钟上限跑一次
   **WIKI-free 的生产模式 `astro build`**(`NODE_ENV=production`,与 dev
   ...
   import 图),引用改写成 `./` 相对、除完全公开外注入 `noindex`,打成
   tar.gz PUT 给网关。分享 id 是 10 位 base58——不含 `0/O/I/l`,念出来不会错。
2. **密码**——只有凭密码的分享才有密码:从作者浏览器发到编辑机一次,在编辑机
   上 scrypt 哈希,发给网关的只有哈希;明文在哪里都不落盘,仅创建成功时显示
   一次。一篇笔记同时只有一个活跃分享:已有活跃分享时再创建会被拒(409)并
   回给现有链接。
3. **改谁能看**——活跃分享可以换成另一种可见性、换密码或换地址:弹层里的
   **改谁能看**一栏送 `POST /share/<id>/visibility`。`/s/<id>/` 是永久句柄,
   始终有效;可读地址换了,旧地址即刻失效,`/s/<id>/` 跳到新地址。换成凭密码
   会生成新密码并显示一次,旧的解锁 cookie 随之失效。只有**变成完全公开**才动
   内容:先按可收录的设置重发快照(此时网关仍按原可见性把守),最后才改网关
   属性——改失败,留下的只是一个没人收录的页面;钉住的分享不能直接改成公开
   (那会把笔记现状发出去),先解钉。其余改动(离开公开、密码与链接之间互换、
   换地址)只改网关属性,不发内容。网关答复不明时,本地记录以网关为准。
4. **跟随**——分享跟着笔记走。笔记改动后安静满 `share.followIdleMinutes`
...
5. **撤销**——删掉网关上的目录,链接立刻 404。本地记录(含 `revokedAt`)
   保留在 `.wiki/data/shares.json` 作审计。

收件人打开链接就读:凭密码的先输密码;完全公开的在 `<publicBase>/<地址>/`
(`/s/<id>/` 会跳过去,id 是永久句柄)。
...
| `PUT /admin/s/<id>` | 带 `x-share-visibility`(或只带 `x-share-password`):创建/替换快照 `<id>`。请求体 = tar.gz,`index.html` 在包根——解压到你以 `/s/<id>/` 提供服务的目录即可。两个头都不带:原地更新已有分享的内容——目录原子替换,可见性、密码、地址与创建时间不变(有效期、笔记 id 没送头也不变);id 不存在答 404 |
| `PATCH /admin/s/<id>` | JSON `{ visibility?, passwordHash?, alias?, expiresAt? }`——只改分享是什么,不碰内容;`null` 清空地址 / 有效期。换到凭密码必须带 `passwordHash`,离开凭密码丢掉哈希,离开完全公开丢掉地址;地址被占答 409 |
| `DELETE /admin/s/<id>` | 删除快照 `<id>`(这里的 404 视为本来就没有);连带释放地址 |
...
| `x-share-visibility` | `password` / `link` / `public`;创建时送。不送而只送 `x-share-password` = `password` |
| `x-share-password` | `scrypt$N$r$p$<salt-b64url>$<hash-b64url>`——N=2¹⁵、r=8、p=1、32 字节哈希。校验访客密码时按内嵌参数重算比对。凭密码时必带,另两种不得带;内容更新时不带 |
| `x-share-alias` | 完全公开分享的可读地址(1–64 个小写字母、数字与中间的连字符);网关在 `/<alias>/` 提供服务,`/s/<id>/` 跳转过去 |
| `x-share-expires` | 可选的 ISO-8601 时间戳;过期后应答 404/410 |
| `x-share-note` | 来源笔记 id(含非 ASCII 时 URI 编码)——仅供参考 |

网关的公开侧按可见性放行:凭密码的 `GET /s/<id>/…` 挡在密码框后面,与存储的
哈希比对;有链接就能看的直接发文件、加 `X-Robots-Tag: noindex`;完全公开的不加
...
`en/chasing/`、`de/chasing/`)。每个普通文件连同字节和 git 模式一起走:可执行
的脚本到了那边还是可执行的。单元以顶层 id 命名,`<unit>/index.{md,mdx}`
...
其他一切——正文、模块、附件——逐字节原样,每个文件保持原来的模式。变换是
确定的,所以**修订号**(revision:对单元文件算的摘要取十六个十六进制字符——
笔记按「frontmatter 排序后的 JSON(不含 `origin`)+ 正文」算,其他文件按
git blob id 算;可执行文件的那一项前面加它的模式 `100755:`,普通文件的
那一项不加)不论谁来算都一样:改 frontmatter 的排版、盖 `origin` 的戳不改变
它,改正文或附件、翻转可执行位一定改变它。
...
3. **把关**在对方的 CI 里跑,每次有推送到 `syndicate/**` 就跑(工作流见
   ...
   只有普通文件(普通或可执行;没有符号链接、子模块、点开头的路径,没有
   同时含 index.md 与 index.mdx 的笔记目录);每篇笔记都带着本次修订号的
   `origin`,文件——字节和模式——摘要等于修订号;单元在 `main` **当前**最新
   提交上的状态仍然允许这次提交
...
编辑器里有 `[[` 自动补全(限定在本篇的语种;输入语种前缀可打开另一语种。
评论渲染数学但不解析双链)。`![[嵌入]]`、引用
...
| `POST /share` | 需登录 | 创建分享——`{note, visibility?, password?, alias?, expiresDays?}`(不写 visibility = 凭密码);NDJSON 流:`progress…` → `result`;该笔记已有活跃分享时 409 |
| `POST /share/<id>/visibility` | 需登录 | 改谁能看——`{visibility, password?, alias?}`,规则同创建;NDJSON 流:`progress…` → `result`;创建者或管理员(否则 403);发布进行中 409 |
