 ...
 * expiry a finite, strictly future timestamp (at exp the session is dead,
 * the same exclusive boundary the jwt path's verifier enforces), user
 * fields strings of sane length, provider from the known set. Kept free of
 * config/server imports so the rules are unit-testable.
 */
