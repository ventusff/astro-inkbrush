...
- ✏️ **原地块编辑** —— Wikipedia 式逐块编辑。CodeMirror 6、跑着你站点自己
  Markdown 插件的服务端实时预览、限定本篇语种的 `[[` 自动补全、乐观锁(别人改过的块会被
  拒绝,绝不覆盖),保存前先过整文件构建关——方言、内容守门、你的插件、
  MDX 编译——过不了就一个字节也不写。页面元信息(frontmatter)也是一个块,
  从页头进去按 YAML 改。
...
- 📤 **分享** —— 把单篇笔记连同全部资源快照成静态包,经一个一下午就能
  实现的小网关 API 对外发布:凭密码、有链接就能看,或在一个可读地址上
  完全公开。
- 🔁 **同步到别的 wiki** —— 把一篇笔记(连同子页、演示、附件与译文)发布到
  另一个 inkbrush wiki,在那边作为一份指回原文的只读副本存在。两个 wiki 之间
  不通话:副本作为一个提交推到对方 git 仓库的暂存分支,对方自己的 CI 用引擎的
  把关脚本处理它——用对方自己的检查、检查的正是将要合入的那个提交——通过就
  推进已发布分支,不通过就写一条退回记录,原文这边读得到。分类值按对方的
  口径逐站映射,到那边会断的链接降级成文字并列出来;每一种状态(已同步、
  有更新、对方改过、待检查、被退回)都从对方仓库的 ref 推出来。
